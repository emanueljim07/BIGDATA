...
class EventsManager {
    ...
    obtenerDataInicial() {
        ...
        $.ajax({
          ...
          success: (data) =>{
            ...
            if (data.msg=="OK") {	   
			  this.poblarCalendario(Object.values(data.eventos))
            }else {
			      if (data.msg=="SD") {	
				  var datos=[{id:1,start:'0000-00-00'}]
				  this.poblarCalendario(datos)
				  }
             // alert(data.msg)
	...
	//  alert((document.getElementById('allDay').checked)?'1':'0');
      form_data.append('allDay', (document.getElementById('allDay').checked)?'1':'0')
